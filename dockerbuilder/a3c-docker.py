...
def get_args():
    ...
    parser.add_argument('--docker', default=False, type=bool, help='docker')
...
def train(shared_model, shared_optimizer, rank, args, info, env):
...
if __name__ == "__main__":
    try:
        ...
        if args.docker: env = grc.RemoteEnv('tmp/sock'); args.num_actions = env.action_space.n
        elif args.sonic: args.num_actions = make(game='SonicTheHedgehog-Genesis', state=args.env).action_space.n # get the action space of this game
        ...
        processes = []
        print("running")
        train(shared_model, shared_optimizer, 0, args, info, env)
        writer.close()
